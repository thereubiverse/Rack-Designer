...
export function AddDevicePicker({ types, templatesByType, initialTypeId = null, onInsert, onClose }: {
  ...
  function openType(id: string) { setActiveTypeId(id); setSelectedId(null); setSearch(""); }
  function backToTypes() { setActiveTypeId(null); setSelectedId(null); setSearch(""); }
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 px-4" role="dialog" aria-label="Add device">
      <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
        ...
        {activeType === null ? (
          /* Level 1 — choose a device type */
          <>
            <p className="mt-0.5 text-sm text-neutral-500">Select type</p>
            <div className="mt-3 max-h-80 space-y-1.5 overflow-y-auto">
              {types.map((t) => (
                <button key={t.id} type="button" data-testid={`picker-type-${t.code}`} onClick={() => openType(t.id)}
                  className="flex w-full items-center justify-between rounded-lg border border-neutral-200 px-3 py-2.5 text-left text-sm font-medium hover:bg-neutral-50">
                  <span>{t.name}</span>
          ...
          /* Level 2 — templates of the chosen type */
          <>
            <button type="button" data-testid="picker-back" onClick={backToTypes}
              className="mt-0.5 flex items-center gap-1.5 text-sm font-medium text-neutral-600 hover:text-neutral-900">
              <span aria-hidden>←</span> {activeType.name}
            </button>
            <div className="mt-4 grid grid-cols-[240px_1fr] gap-4">
              <div className="flex max-h-72 flex-col gap-2">
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search"
                  className="h-9 w-full rounded-lg border border-neutral-200 px-3 text-sm focus:border-neutral-400 focus:outline-none" />
                <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto rounded-xl border border-neutral-200 p-2">
                  {filtered.length === 0 && (
                    <p className="p-4 text-center text-sm text-neutral-400">
                      {templates.length === 0 ? `No ${activeType.name} templates yet.` : "No matches."}
                    </p>
                  )}
                  {filtered.map((t) => (
