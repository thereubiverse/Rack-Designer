...
TUNNEL_COMPOSE_FILE="$SCRIPT_DIR/docker-compose.tunnel.yml"
...
# ---------------------------------------------------------------------------
# Step 2: how the server is reached, hostname, first admin, password
#
# The mode is asked FIRST because it changes what the hostname has to be — a direct install needs DNS
# pointing at this machine before Caddy can get a certificate; a tunnel install needs the public
# hostname routed to the tunnel and never touches DNS on this machine at all.
#
# Asked once and recorded in deploy/.env, so a re-run does not ask again (the script's header
# promises it is safe to re-run, and quietly switching a live deployment from tunnel to direct
# because someone hit Enter would break it in a way that looks like a network fault).
# ---------------------------------------------------------------------------
log "Step 2: configuration"

DEPLOY_MODE="$(env_get DEPLOY_MODE)"
if [[ -n "$DEPLOY_MODE" ]]; then
  echo "Using existing deployment mode: $DEPLOY_MODE"
else
  cat <<'EOF'

How will this server be reached from the internet?

  1) direct  — this machine has a public IP and ports 80 and 443 reach it (port forwarding, or a
               cloud VM). Caddy publishes both and obtains its own Let's Encrypt certificate.
  2) tunnel  — Cloudflare Tunnel. Nothing is published to this machine and no inbound port is
               opened; a cloudflared container connects outbound to Cloudflare. Choose this behind a
               home or office router, on a dynamic IP, or where you cannot forward ports.

EOF
  read -r -p "Choose 1 or 2 [1]: " mode_choice
  case "${mode_choice:-1}" in
    1 | direct) DEPLOY_MODE=direct ;;
    2 | tunnel) DEPLOY_MODE=tunnel ;;
    *) die "'$mode_choice' is not one of 1 (direct) or 2 (tunnel)" ;;
  esac
  env_set DEPLOY_MODE "$DEPLOY_MODE"
fi

case "$DEPLOY_MODE" in
  direct | tunnel) ;;
  *) die "DEPLOY_MODE in deploy/.env is '$DEPLOY_MODE' — it must be 'direct' or 'tunnel'" ;;
esac

if [[ "$DEPLOY_MODE" == "tunnel" ]]; then
  [[ -f "$TUNNEL_COMPOSE_FILE" ]] || die "tunnel mode needs $TUNNEL_COMPOSE_FILE, which is missing"
  [[ -f "$TUNNEL_CADDYFILE" ]] || die "tunnel mode needs $TUNNEL_CADDYFILE, which is missing"

  # Rebuild rather than append, so the two -f flags are always in base-then-override order — compose
  # applies later files on top of earlier ones, and the override is the one that must win.
  COMPOSE=(
    docker compose
  ...
  # -s: the token is a bearer credential for the tunnel and is treated like a password. It goes into
  # deploy/.env (mode 600) via env_set, which is a shell builtin printf — so it never reaches an
  # argv, never appears in `ps`, and is never echoed back.
  CLOUDFLARE_TUNNEL_TOKEN="$(env_get CLOUDFLARE_TUNNEL_TOKEN)"
  if [[ -n "$CLOUDFLARE_TUNNEL_TOKEN" ]]; then
    echo "Using the Cloudflare Tunnel token already in deploy/.env."
  else
    cat <<'EOF'

Cloudflare Tunnel token. Get it from one.dash.cloudflare.com > Networks > Tunnels > Create a tunnel
> Cloudflared; it is the long value after `--token` in the install command Cloudflare shows you.
Route the tunnel's public hostname to the service http://caddy:80.

EOF
    read -r -s -p "Cloudflare Tunnel token: " CLOUDFLARE_TUNNEL_TOKEN
    echo
    [[ -n "$CLOUDFLARE_TUNNEL_TOKEN" ]] \
      || die "tunnel mode needs CLOUDFLARE_TUNNEL_TOKEN. Create the tunnel in the Cloudflare dashboard, then either re-run this script or put the token in deploy/.env as CLOUDFLARE_TUNNEL_TOKEN=..."
...
#
# In TUNNEL MODE there is an ordering constraint before any of that, and it has to be enforced here
# rather than written down and hoped for.
#
# The app container mounts Caddy's internal root CA as a FILE and points NODE_EXTRA_CA_CERTS at it,
# which is what lets its server-side calls to https://${APP_HOSTNAME} stay on the compose bridge.
# Caddy does not generate that root until its first boot. If the app starts first, Docker sees a
# bind-mount source that does not exist and creates a DIRECTORY there — the mount then succeeds,
# NODE_EXTRA_CA_CERTS points at a directory, Node ignores it without a word, and the app fails with
# UNABLE_TO_GET_ISSUER_CERT_LOCALLY exactly as if the trust had never been configured. Nobody can
# sign in, and the evidence on disk looks fine.
#
# So: start caddy ALONE (--no-deps, because caddy depends_on app and would otherwise drag it up),
# wait for root.crt to exist as a file, and only then start everything else.
# ---------------------------------------------------------------------------
if [[ "$DEPLOY_MODE" == "tunnel" ]]; then
  ...
  # as uid 1000 (`node`, from the Dockerfile). On Linux, where a bind mount keeps the real owner, uid
  # 1000 therefore cannot open it. Node does NOT fail loudly on an unreadable NODE_EXTRA_CA_CERTS: it
  # prints a warning and carries on without those certificates, which lands on exactly the
  # UNABLE_TO_GET_ISSUER_CERT_LOCALLY this whole arrangement exists to prevent. (Docker Desktop on
  # macOS remaps bind-mount ownership to the host user, so this failure does NOT reproduce there —
  # it appears only on the machines this actually ships to.)
  #
  # chmod runs INSIDE the caddy container, which is root there. Doing it on the host would fail: the
  # operator running this script does not own a file the container created as root.
  "${COMPOSE[@]}" exec -T caddy chmod 0644 /data/caddy/pki/authorities/local/root.crt \
    || die "could not make Caddy's root certificate readable by the app container — check '${COMPOSE[*]} logs caddy'"

  log "Caddy's internal root CA is in place"
fi
...
if [[ "$DEPLOY_MODE" == "tunnel" ]]; then
  cat <<EOF
Cloudflare Tunnel mode: no port on this machine is published, and nothing needs forwarding at your
router. Check the tunnel shows Healthy at one.dash.cloudflare.com > Networks > Tunnels, and that its
public hostname $APP_HOSTNAME is routed to the service http://caddy:80.

Every 'docker compose' command for this deployment needs BOTH files, in this order:
  docker compose -f deploy/docker-compose.yml -f deploy/docker-compose.tunnel.yml ...
Leaving the second one off starts a stack that publishes 80/443 and has no tunnel.

EOF
fi

cat <<EOF
Still to configure, all optional (edit deploy/.env, then '${COMPOSE[*]} up -d'):
